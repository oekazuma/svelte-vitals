...
Both numbers were previously guesses. They are now derived by surveying real Svelte 5 codebases and taking the median of each repository's 90th percentile — the same benchmark-based method ReactSniffer uses for React — and both held when the survey was widened. At the old values these rules almost never fired on a typical Svelte project.
