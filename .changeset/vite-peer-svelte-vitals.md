...
With peers there is one shared copy: the plugin validates and analyzes with the same rule registry the CLI runs, and installing versions outside the plugin's supported range surfaces as an install-time peer warning. npm and pnpm auto-install missing peers; if you install with yarn, add `svelte-vitals` as a devDependency alongside `@svelte-vitals/vite`.
