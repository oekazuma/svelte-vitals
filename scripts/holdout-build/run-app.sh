...
git submodule update -q --init --recursive --depth 1 >>"$log" 2>&1 || { code=$?; first_look uninstalled; result submodules failed $code; exit 0; }
...
case "$lock" in
  ...
  # Only a lockfile out of sync with package.json (EUSAGE) falls back to `npm install`, which resolves it
  # as pnpm's --no-frozen-lockfile does. npm reports a failing install script (the app's own postinstall,
  # a native build) as "command failed"; the retry installs the same packages without scripts.
  package-lock.json) npm ci >>"$log" 2>&1 ||
    { grep -q "npm error code EUSAGE" "$log" && npm install --no-audit --no-fund >>"$log" 2>&1; } ||
