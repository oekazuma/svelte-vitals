...
- **Agent Skills** — `/svelte-vitals`, `/improve-svelte`, and `/setup-svelte-vitals` slash-command skills for Claude Code, Cursor, and Codex: rule knowledge up front, a project-wide, evidence-ranked improvement roadmap, and a first-run pass that derives the config file from what the project already declares. Install with `npx skills add oekazuma/svelte-vitals` ([skills.sh](https://www.skills.sh/)). → [Agent Skills](https://oekazuma.github.io/svelte-vitals/guides/agent-skills/)
