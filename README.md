...
- **Bundled docs** — `svelte-vitals docs list` / `docs show <name>` print the guides from inside the CLI, so they match the installed version and need no network; `svelte-vitals explain --list` / `explain <rule-id>` do the same for the rule set. Both take `--json`. → [CLI reference](https://oekazuma.github.io/svelte-vitals/guides/cli/)
