...
**Health スコア**は、分析結果に存在する全カテゴリにわたるプロジェクトの総合的な品質を、0〜100 の単一の数値に要約したものです。CI パイプラインをゲートするために `--min-health` フラグで使用されます。
...
アクティブな各カテゴリ（例：SEO、パフォーマンス）に対して、svelte-vitals は独立したスコアを計算します：
...
**結果に存在するカテゴリのみが含まれます。** 例えば、パフォーマンスルールに1件も該当しないプロジェクトでは、パフォーマンスカテゴリは結果を生成せず、Health は残りのカテゴリのみに基づいて算出されます。
