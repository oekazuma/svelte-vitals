...
| カテゴリ       | 全カテゴリ（SEO、Performance、Correctness、Security、Architecture、Accessibility） | 全カテゴリ（SEO、Performance、Correctness、Security、Architecture、Accessibility） | 全カテゴリ（静的ベースライン）、訪問済みルートはレンダリング済みSEO/Performanceの精度に精緻化 |
...
ビルドモードは `vite build` の実行中に**実際にプレレンダリングされた HTML** を解析して SEO/Performance/Accessibility を検証します。何が生成したかに関わらず、タグが出力 HTML になければ検出され、`failOn` の閾値に達した時点でビルドが失敗します。加えて `.svelte` ソースも走査し、CLI と同じ Correctness・Security・Architecture と、コンポーネントスコープの Performance ルールを実行します。
