...
| コードを書く前からエージェントにルールを教える、改善ロードマップを作らせる、または導入時に設定を導出させたい | **Agent Skills**：`/svelte-vitals`、`/improve-svelte`、`/setup-svelte-vitals` |
...
[Agent Skills](/ja/guides/agent-skills) は、エージェントが「コードを書く前からルールを知っている」状態を作ります。`npx skills add oekazuma/svelte-vitals` は Claude Code・Codex・Cursor で同じように動く `SKILL.md` をインストールします。**`/svelte-vitals`** はルールカタログと「編集のたびに実行する」プレイブック、**`/improve-svelte`** は「アプリをレビューして」を影響度順の実装プランに変える読み取り専用の監査スキル、**`/setup-svelte-vitals`** は導入時のスキルで、プロジェクトが既に宣言している内容から `svelte-vitals.config` を導出し、各ルールを採用する前に必ず計測します。
