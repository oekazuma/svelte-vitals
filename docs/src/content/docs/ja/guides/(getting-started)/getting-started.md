...
CLI はソースコードだけで完結します。レイアウトチェーンをたどりながら `<svelte:head>` ブロックを解析して各ルートの実効 `<head>` を解決し、コンポーネント本体を読むルールもそのまま実行します。ページ自身の `load` が常にリダイレクトするか SvelteKit の `error()` を投げる場合（`+page.ts` または `+page.server.ts` の `load` のどの経路も `redirect()` か `error()` に到達する場合。`if`/`else` の両方の分岐、`catch` がリダイレクトまたは再 throw する `try`、常にそうなる同じファイル内の関数の呼び出しもたどります）、そのページ自体は描画されないため、ルート単位の検査は対象外です。コンポーネントのルールは、そのファイルも読みます。実行中のサイトもブラウザも必要ありません。プリレンダリングされたページについては、[Vite プラグイン](/ja/guides/plugin-mode)が同じルールを「実際にビルドされた HTML」に対して実行します。動的な値が残らないぶん、こちらのほうが正確です。
