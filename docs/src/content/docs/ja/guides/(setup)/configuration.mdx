...
一つ例外があります。`--rules` は `rules` をキー単位でマージするのではなく、全体を丸ごと置き換えます — コマンドラインで指定すると、その実行ではフラグから構築されたルールセットが設定ファイルの `rules` を丸ごと置き換えます。`--ignore` はこれとは違い、名前を挙げたルール ID に対して `off` エントリを追加するだけです。これは `rules`（設定ファイル、あるいは `--rules` による置き換え後の値）の上にレイヤーとして重ねて適用されるため、名前を挙げていないルールは設定ファイルで指定された重大度やオプションをそのまま保持します。
