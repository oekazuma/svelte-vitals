...
- 一部のルール — 現時点では [`seo/title-length`](/ja/rules/seo/title-length)、
  [`seo/description-length`](/ja/rules/seo/description-length)、
  [`performance/preconnect`](/ja/rules/performance/preconnect) — では、成功した検出結果にファイルの位置情報が含まれないため、`files:` でスコープした `'off'` はそのルールの失敗した検出結果は取り除けても、成功したシード(passing seed)は取り除けず、スコアが動く余地が残ります。`route:` でスコープした `'off'` はこの影響を受けません。この不整合が解消されるまで、これらのルールには `files:` ではなく `route:` を使ってください([#382](https://github.com/oekazuma/svelte-vitals/issues/382))。
