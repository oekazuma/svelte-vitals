...
特定の設定ファイルを指定して分析したい場合は `--config <path>` を使います。指定すると探索は行われず、その設定ファイルだけが使われます（マージはされません）。受け付ける拡張子は `.js` と `.ts` のみで、ファイルが存在しない・読み込めない場合は exit `2` で終了します。設定をコミットする前に試したいとき、モノレポ内の複数アプリで一つの設定を共有したいときに使えます。
