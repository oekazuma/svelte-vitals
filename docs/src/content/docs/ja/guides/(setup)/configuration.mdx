...
無効になるのは `seo/canonical-url`・`seo/og-title`・`seo/og-description`・`seo/og-image`・`seo/og-url`・`seo/twitter-card`・`seo/json-ld`・`seo/sitemap-xml`・`seo/sitemap-in-robots`・`seo/title-length`・`seo/description-length` です。`rules` でそれぞれを `'off'` と書いた場合とまったく同じ扱いになり、Health スコアの母数からも外れます。
