...
`ref` と比較して**新規に**追加された検出結果のみを報告します。つまり、`ref` を同じように解析したときには存在しなかった検出結果です。ファイル単位でスコープする `--diff`/`--staged` とは異なり、`--baseline` は検出結果の同一性でスコープします。そのため、変更したファイルに元からあった問題でゲートが失敗することはなく、その変更が実際に**導入した**問題だけが対象になります。デフォルトの `ref` はなく、明示的に指定します。
