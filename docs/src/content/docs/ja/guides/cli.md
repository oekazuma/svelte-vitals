...
> どちらのフラグも、検出結果をそのソースファイルの場所でフィルタリングします。分析対象のプロジェクトが git リポジトリのサブディレクトリ(例:モノレポの `apps/web/`)にある場合でも正しく動作します。git が利用できない場合(または `ref` が無効な場合)、svelte-vitals は警告を表示し、プロジェクト全体を分析します。
