...
`srcset` 属性のない `<img>` を検出します。`<picture>` のフォールバックの `<img>` で、その `<source>` 要素に `srcset` がある場合は検出しません。ブラウザがその候補から画像を選ぶためです。

SVG は検出しません。ベクター画像は 1 つのファイルのままどのサイズにも拡大縮小できるためです。対象は、`src` のパスが `.svg` で終わるもの（クエリとフラグメントは無視）、インラインの `data:image/svg+xml` URI、そしてソース解析では `.svg` ファイルから import した `logo` を渡す `src={logo}` です。
