...
ルートのマークアップで最初に現れる `<img>` に `loading="lazy"` が付いている場合に検出します。静的解析では、文書順で最初の画像を Largest Contentful Paint（LCP）画像とみなします。`{#snippet}` 内の画像は、定義の位置ではなく、そのファイルで最初に `{@render}` される位置にあるものとして扱います。コンポーネントに渡すだけのスニペットは、定義の位置のままです。
