...
検出は `performance/load-waterfall` と同じ保守的な直線走査です（束縛と中間定数を通じた前方 taint 伝播、コールバック引数のシャドーイングを考慮、`await parent()` は対象外）。作成済みの Promise を await するだけの箇所（`await somePromise`、`await store.ready`）はリクエストを開始しないため対象外です。処理を開始するとみなすのは、関数呼び出し・タグ付きテンプレート・`new`・`import()` を await する箇所だけです。
