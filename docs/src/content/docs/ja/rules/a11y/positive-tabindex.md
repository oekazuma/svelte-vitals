...
- 空または非数値の `tabindex` — 不正な HTML で、ブラウザに無視されます。

## なぜ重要か

正の `tabindex` を持つ要素は、ページ内の自然順のすべての要素より前に来ます。そのためたった1つの `tabindex="1"` がドキュメント全体のキーボードナビゲーションを組み替えてしまい、被害は膨らみ続けます: 後から追加された正の `tabindex` を持たない要素は、視覚上どこにあってもすべてその後ろに並びます。タブ順は視覚上の順序から乖離し、WCAG 2.4.3 (Focus Order) が求める意味のあるフォーカス順が壊れます。安全な値は `0` と `-1` だけというのが広く合意されたガイダンスで、正の値が意図的かつ正しいことはまずありません。
...
対象になるのはネイティブ要素のリテラルな `tabindex` 値だけです。式による `tabindex`、それを供給しうるスプレッド属性、`<svelte:element>` による動的タグは静的解析の範囲外のため検出されません。
