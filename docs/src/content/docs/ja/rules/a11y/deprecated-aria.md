...
ARIA 1.3 の表に照らして 3 つを、コンポーネントのソースから検出します。CLI と Vite プラグインの両方が対象です。`--route` で実行範囲を絞ると、このルールは動きません — コンポーネントスコープのルールには、検出を紐づけるルートが無いためです。
...
検出しないもの: `a11y/unknown-aria-attribute` が既に報告する属性。DPUB-ARIA の role、式による role、literal な role のない spread の上のもの。
