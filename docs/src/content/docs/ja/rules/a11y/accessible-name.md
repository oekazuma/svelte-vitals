...
- `aria-hidden="true"`、`hidden` 属性、`role="presentation"`/`role="none"` を持つ iframe — 名前が誰の役にも立たない、隠しトラッキング/解析フレームの類型です。これらの属性が式値の場合は判定不能 → 検出しない、というルール全体の規約どおりです。このスキップは iframe の腕だけに適用されます: 隠れた無名ボタンは引き続き報告されます。
...
button/link/画像ボタンの腕にはコンパイラの対応物がありません(`a11y_missing_content` が対象にするのはアンカーと見出しだけで、根拠も異なります)。
