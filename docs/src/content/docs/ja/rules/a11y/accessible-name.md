...
- `<label>` が名前を与えている `<button>` と `<input type="image">` — 囲んでいる場合と、`for` で `id` を指している場合の両方。名前の計算では、この label の段階が要素自身のサブツリーより先に来ます。`<a>` にこの段階は無いため、リンクは内容だけで判定します。`for` 経由の判定は同一ファイル内に限られ、他コンポーネントの label は既知の制限です。
