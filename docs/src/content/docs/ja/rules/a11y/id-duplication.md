...
- **ソース解析**（CLI、ダッシュボードの静的ベースライン） は、`src/app.html` の id と衝突するページ側の id も報告しません。シェルの id は、参照先の候補としては読みますが、数える対象の出現としては扱わないためです。
- **レンダリング解析**（Vite プラグインのビルド、ダッシュボードで訪問したルート） は最終的なプリレンダリング済み HTML を読みます。そのため、実際にレンダリングされた id だけを認識します。シェルの id との衝突も、`{#each}` が生成した id もすべてここに含まれます。ループ由来の実際の重複は、このモードでのみ表面化します。ソースファイルを持たないため、検出結果の位置は特定のファイル・行ではなくルート自体に紐づきます。同じ不具合でも、永続化される検出キーはモードによって異なります。
