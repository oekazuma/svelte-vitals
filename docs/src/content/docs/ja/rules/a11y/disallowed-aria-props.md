...
要素の role が**禁止する**、または**所有しない** `aria-*` 属性を、ARIA 1.3 の role 表に照らしてコンポーネントのソースから検出します。CLI と Vite プラグインの両方が対象です。`--route` で実行範囲を絞ると、このルールは動きません — コンポーネントスコープのルールには、検出を紐づけるルートが無いためです。
