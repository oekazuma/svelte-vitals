...
- 名前空間は追跡していません。`<svg>` 内の `<a href>` は HTML の `<a>` として分類されるので、`<button>` を包む SVG のリンクはインタラクティブ要素のネストとして報告されます。これはパーサの近似であって SVG の制約ではありません — SVG 自身が禁じるのは SVG リンクのネストだけです — が、HTML ルールの読者も望まないパターンなので、この報告は残しています。
