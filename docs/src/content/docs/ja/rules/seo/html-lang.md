...
**ソース解析**（CLI、ダッシュボードの静的ベースライン）は `src/app.html` の `<html lang>` 属性を読みます。**レンダリング解析**（Vite プラグインのビルド）は出荷される文書の `<html lang>` — それを持つ最初のプリレンダリング済みページ — を読むため、`handle` がレンダリング時に埋める値はそちらで判定されます。ダッシュボードのライブ層はこのルールを評価せず、静的ベースラインの結果がそのまま残ります。
