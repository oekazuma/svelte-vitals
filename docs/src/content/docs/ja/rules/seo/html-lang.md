...
**ソース解析**（CLI、ダッシュボードの静的ベースライン）は `src/app.html` の `<html lang>` 属性を読みます。**レンダリング解析**（Vite プラグインのビルド）は出荷される文書の `<html lang>`、つまりそれを持つ最初のプリレンダリング済みページの値を読むため、`handle` がレンダリング時に埋める値はそちらで判定されます。ソース解析が見るのは `app.html` に書かれたままの属性だけです。ダッシュボードのライブ層はこのルールを評価せず、静的ベースラインの結果がそのまま残ります。
