...
- **ソース解析**（CLI、ダッシュボードの静的ベースライン）はルートの `.svelte` テンプレートを走査します。そのため、`{#if false}` のようなレンダリングされない分岐の中にある見出しも数える一方、インポートした子コンポーネントが描画する見出しは見えません。1 つの `{#if}`/`{:else if}`/`{:else}` や `{#await}` ブロックの各分岐は別々に読みます。見出しは、同時に描画されうる直前の見出しと比べ、隣の分岐の見出しとは比べません。別々のブロックはすべて描画されるものとして扱います。
