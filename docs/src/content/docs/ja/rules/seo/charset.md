...
**レンダリング解析のみ**（Vite プラグインのビルド、ダッシュボードで訪問したルート）。このタグは `src/app.html` にあります。ソース解析（CLI、ダッシュボードの静的ベースライン）はそこからタイトル、canonical リンク、ほかの `<meta>` タグを読みますが、このタグは読まないため、ソース解析ではこのルールは何も報告しません。
