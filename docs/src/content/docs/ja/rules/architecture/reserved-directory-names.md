...
`src/lib/Card.v2/Card.v2.svelte` は受け入れられます —— `Card.v2.svelte` の最初のドットまでの
語幹は `Card` であり、ディレクトリ自身の（カットしていない）名前 `Card.v2` とは一致しないため、
このルールはこれをユニットとみなしません。`architecture/unit-entry-file` は明示的な拡張子で
...
名前が出ます。glob が 1 つのディレクトリにもマッチしなかった。マッチしたディレクトリがすべて
除外されていた。ディレクトリにはマッチしたがユニットには一度もマッチしなかった。値が名前を 1 つも
列挙していなかった。同じ glob が両方のマップに宣言されていた。

意図的にまったく報告されないものが 2 つあります。`overrides` エントリの**中だけ**で宣言したキーは
この無効性の検査を受けません。何にマッチしたかが、そのオーバーライドの適用範囲に依存するためです
—— ただし 1 つ例外があります。同一 glob の衝突検査はグローバルに宣言されたキーに絞られていないため、
`overrides` エントリだけから組み上がった `scopes`/`unitScopes` の衝突は、それでも報告されます。
また、現在どのディレクトリも使っていない宣言済みの名前は報告されません —— この集合が表すのは
現れてよいものであって、現れなければならないものではないからです。
