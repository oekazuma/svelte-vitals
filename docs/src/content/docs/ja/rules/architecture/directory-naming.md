...
名前が記法に適合しているかを問うているのではないからです。`architecture/reserved-directory-names`
も同じ先頭 1 文字のテストを自分のユニット定義に使いますが、それに加えて同名のファイルを要求するため、
一方のルールの PascalCase 判定を通っても、もう一方では通らないディレクトリがあり得ます。3 つの
ルールは、意図的に同じ言葉で異なることを意味しています。
