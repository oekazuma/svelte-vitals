...
間違いであり、必ず報告されます** —— 素のパスが存在していて単にそのマップの求めるユニットでない
場合は「ユニットに一度も到達しない」として、1 つのディレクトリにもマッチしないか除外された
ディレクトリにしかマッチしない場合はより前の理由として報告されます。`src/lib` ではなく
`src/lib/**` と書いてください。
...
レイヤーは除外を追加することしかできないため、これによって診断が静かになることはあっても、うるさく
なることはありません。
