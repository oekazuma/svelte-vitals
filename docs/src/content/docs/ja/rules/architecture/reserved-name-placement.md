...
- ユニットマップの glob が実在するディレクトリにマッチしたが、そのどれも求める種類のユニットでは
  ...
  これが上で述べた素の glob の書き間違いを検出するのは、予約名が glob 自身のディレクトリの**直下**に
  ある場合だけです。`parts: 'src/lib'` は `src/lib/Card/parts` について何も言いません。検査対象は
  その `parts/` の親である `src/lib/Card` であり、glob はそこにマッチしないからです。`src/lib` では
  なく `src/lib/**` と書いてください。
