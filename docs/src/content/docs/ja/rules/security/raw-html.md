...
汎用の HTML サニタイザーは、HTML ではないペイロードには不向きなツールでもあります。たとえば JSON-LD の `<script>` の中身は HTML ではなく、信頼できる JSON シリアライズ済みデータから組み立てているなら `{@html}` 自体が不要な場合もあります。こうした使用箇所は個別にレビューし、値にユーザー入力が混ざり得ないと確認できるなら、見当違いなサニタイザーを通さずに抑制してください。
