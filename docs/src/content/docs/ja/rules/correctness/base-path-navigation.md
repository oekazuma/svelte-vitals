...
設定側で値を計算している場合、よくある `base: dev ? '' : '/repo'` のような形も対象です。少なくともどれかの環境では base 配下で配信されるためです。base が無い場合、明示的に `base: ''` の場合、計算式のどの分岐もリテラルの `''` になる場合（`dev ? '' : ''`）は発火しません。

`base: '/docs'` の下の `href="/docs/about"` のように、リテラルの base で始まっているリテラルはアプリ内を指すため報告しません。ただし base を変えると壊れるので、移植性のある書き方は引き続き `resolve()` です。base が計算値の場合は先頭が base と一致するか判定できないため、ルート相対のリテラルはすべて報告します。
