...
- **定数リスト**。インラインの配列リテラルと、コンポーネント自身のスクリプトにある `const` の配列リテラルのうちファイル内で変更される余地がないもの、そうしたリテラルをプロジェクト内のモジュールから import したもの（`correctness/each-key` を参照）です。並べ替えが起きないため、index が安定した同一性になります。
