...
`$state` フィールドを持つクラスのインスタンスを受け取った prop も、フィールドやメソッド経由で変更すると（`todo.done = true`、`cart.add(item)`）、その変更がリアクティブであっても検出されます。静的解析ではクラスのインスタンスとプレーンなオブジェクトを区別できないためです。意図したパターンであれば、その箇所をインラインで抑制してください。
...
`delete items.x` も同じです。メンバーへの代入や更新（`user.name = …`、`user.count++`）は事情が異なります。それ自体が代入なので、コンパイラが prop の更新に変換します。そのため legacy mode では検出しません。
...
同じ関数の中で prop を再代入していれば、変異メソッド呼び出しは検出しません。別の関数での再代入は対象外です。
