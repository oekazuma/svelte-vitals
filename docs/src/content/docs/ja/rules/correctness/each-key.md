...
- 同じリテラルをコンポーネント自身のスクリプトで名前付きにしたもの（`const tabs = [ … ]`、export なし）。ただしファイル内で変更される余地がない場合に限ります。つまり、ほかの参照がすべてメンバーの読み取り（`tabs.length`）、`{#each tabs …}`、`for (const t of tabs)` のループのいずれかである場合です。変更するメソッド呼び出し（`tabs.push`）、それを介した代入、`tabs` の受け渡しやスプレッドがあれば検出対象のままです。
