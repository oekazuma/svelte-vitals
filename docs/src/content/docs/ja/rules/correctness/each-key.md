...
- 同じリテラルをプロジェクト内のモジュールから import したもの（`export const TABS = [ … ]`。`as const` や `satisfies` 付きも含む）。`$lib` などの `kit.alias`、相対パス、バレルの再エクスポートを経由しても解決し、`TABS` としても `import * as ns` の `ns.TABS` としても回せます。モジュール内で書き換えられておらず、コンポーネント側も上記の読み取りだけである場合に限ります。パッケージや JSON ファイルからの import、配列リテラルではない export（`LIST.map(…)` やスプレッドを含むもの）は検出対象のままです。
