...
`@svelte-vitals/vite` piggybacks on `vite build`, parses the **prerendered HTML**, and runs the same SEO, Performance, and Accessibility rules as the CLI, apart from `a11y/doctype`, which reads `src/app.html` and is CLI-only. Inspecting the real output makes it library-agnostic.
...
| `metaComponents` | `string[]`                                                   | —            | Head-metadata components the analyzer cannot resolve — resolvable in-repo components are followed automatically |
