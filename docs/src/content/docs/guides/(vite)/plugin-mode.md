...
A `svelte-vitals-suppressions.json` next to it (written by [`svelte-vitals --update-suppressions`](/guides/cli#svelte-vitals-suppressionsjson----update-suppressions----no-suppressions)) is applied too, by the build gate and by the live dashboard's whole-project layer, so both agree with the CLI. Only source-scan entries apply (Correctness, Security, Architecture, component-level Accessibility — the findings whose rule id, route and location the plugin shares with the CLI). A rendered route finding anchors to the built HTML file, not the `+page.svelte` the CLI records, so the plugin skips route-level entries on every surface, build gate and dashboard alike, and prints how many it skipped — use [`overrides`](/guides/configuration#overrides) for those. The build also prints how many findings the file suppressed; stale entries are reported by the CLI only. A malformed file fails the build, as it fails the CLI.

## Scope

The HTML check covers prerendered routes only. Routes that are not prerendered (server-rendered, or dynamic routes without prerendered entries) have no build output, so nothing here reads their rendered HTML. A prerendered route with `ssr = false` has build output, but it is the empty app shell, so the HTML check skips it too — a `+layout` with `ssr = false` skips its whole subtree — and the build prints one warning listing the skipped routes. A route that redirected during prerendering is written as SvelteKit's redirect stub rather than a page, so the HTML check skips it without a warning. That includes a conditional redirect that happened to fire at build time, which the CLI still checks from source. The `svelte-vitals` CLI covers those routes by source analysis instead, and the [live dashboard](/guides/dev-dashboard) gets a rendered reading of them when you browse during `vite dev`. The source scan applies project-wide regardless of how a route renders. When the build prerenders nothing at all (an SPA served from a fallback page, say), the plugin prints one warning that route analysis was skipped and still runs and gates the source scan; `seo/html-lang`, which reads the rendered `<html>`, is skipped with it.
