...
`svelteVitalsHandle` observes each request's fully-rendered `<head>` via `transformPageChunk`, fire-and-forget: it never modifies or delays the response and swallows its own errors, so it cannot break the dev server.

On a visited route, a rendered result replaces the static one **for the same rule id** — closer to the truth, especially for dynamic values. The route's other static findings, routeless (component- and site-scoped) findings, and unvisited routes stay as they were. Route headings carry a provenance badge: `measured` once a route has a rendered layer, `static` otherwise.
