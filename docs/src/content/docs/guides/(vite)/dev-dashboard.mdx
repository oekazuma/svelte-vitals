...
A visited route's rendered results replace its static ones — closer to the truth, especially for dynamic values. Route headings carry a provenance badge: `measured` from a real rendered page, `static` from source analysis only.
