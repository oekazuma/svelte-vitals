...
last one set wins. A **list** or **map** option instead **adds** to the built-in set, never
replacing it, so a project keeps benefiting from new built-in entries in later svelte-vitals
releases instead of being frozen at the list as of the day it wrote its config.
