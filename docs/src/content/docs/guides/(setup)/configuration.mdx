...
`seo/title-length`, and `seo/description-length` — the same effect as writing each one as
`'off'` in `rules`, including their removal from the Health denominator. It also turns off
`seo/indexability`, since the switch already says the `noindex` is intentional.

Two SEO rules stay on. `seo/single-h1` checks document structure, which assistive technology
reads whether or not the page is indexed, and `seo/robots-txt` still does real work: a `noindex`
meta tag is only read after the page has been crawled. The JSON-LD validity rules stay on too —
a structured-data document that exists should be correct.
