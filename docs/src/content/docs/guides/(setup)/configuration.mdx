...
entirely rather than merged with it. It accepts `.js` and `.ts` only, and a missing or unreadable
file exits `2`. Reach for it to try a config out before committing it, or to share one config
