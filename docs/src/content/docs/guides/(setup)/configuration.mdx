...
`install --client config-file` picks `.ts` only when all three hold: the current Node loads it natively, the project looks TypeScript-oriented (`tsconfig.json` or `vite.config.ts` at the root), **and** `svelte-vitals` is a declared dependency — the `defineConfig` import resolves at load time, so an npx-only project must not get it.

Otherwise it writes `.mjs`, which works on every supported Node with no flag and no dependency. `--force` keeps an existing file's extension rather than switching format underneath you.
...
- For a small number of rules — currently [`seo/title-length`](/rules/seo/title-length),
  [`seo/description-length`](/rules/seo/description-length), and
  [`performance/preconnect`](/rules/performance/preconnect) — a passing result carries no file
  location, so a `files:`-scoped `'off'` removes their penalized findings but not their passing
  seed, and the score can still move. `route:`-scoped `'off'` is unaffected. Prefer `route:` over
  `files:` for these rules until the underlying inconsistency is fixed
  ([#382](https://github.com/oekazuma/svelte-vitals/issues/382)).
