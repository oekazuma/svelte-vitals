...
One exception: `--rules` replaces `rules` as a whole, not merged key-by-key — pass it on the command line and the flag-built rule set replaces the config file's `rules` entirely for that run. `--ignore` is different: it only ever adds `off` entries for the rule ids it names, layered on top of whatever `rules` already resolved to (the config file, or `--rules`'s replacement) — a rule it doesn't name keeps its file-configured severity and options.
