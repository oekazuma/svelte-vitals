...
Otherwise it writes `.js`. Both files are ESM — the project must be `"type": "module"`, which is SvelteKit's default; CommonJS projects are not supported. `--force` keeps an existing file's extension rather than switching format underneath you.

Pass `--config <path>` to analyze under a specific config file instead — discovery is skipped
entirely rather than merged with it. A relative path resolves against the directory you run the
...
`defineConfig` is a thin helper that merges your object over the built-in defaults; it buys type-checking and autocomplete in a `.ts` file and nothing else. A bare `export default {...}` behaves identically at runtime — and, unlike the import, works when svelte-vitals is only ever run via `npx`.
...
| `metaComponents` | `string[]`                                                                              | `[]`               | `<head>`-metadata components the analyzer cannot resolve (e.g. from an npm package) — resolvable in-repo components are followed automatically |
...
Layers combine in order — built-in default, `rules`, then each matching `overrides` entry — and how
they combine depends on the option kind:

- **integer** — replaces; the last one set wins.
- **list** / **map** — **adds** to the built-in set rather than discarding it, so later
  svelte-vitals releases can still grow the defaults under you.
- **map**, existing key — keeps the entry but takes your value, which is how you reword the
...
- [`architecture/doc-link-target`](/rules/architecture/doc-link-target) — a `urlRoots` list of URL
...
or another custom alias, not just `$lib` or a relative import) — so every SEO rule reading that channel
sees content the same way regardless of which alias reached it.
...
- an alias whose value is computed (`path.resolve(...)`, a template literal) rather than a plain string —
  the specifier is treated as one we cannot see, so nothing is reported for it;
- an alias whose value is a literal absolute path — a POSIX path like `/opt/shared/src`, or a Windows
  drive-letter path like `C:\shared\src` — since that names a file outside the analyzed project;
...
- a project that passes its SvelteKit options to the `sveltekit()` plugin in `vite.config.ts` instead of
  `svelte.config.{js,ts}` — same result, nothing reported;
- a `kit.alias` object containing a spread (`{ ...shared, '$a': 'src/a' }`) or a computed key
  (`{ [key]: 'src/a' }`) — an unknown key could shadow any entry declared after it, so the _whole_ of
  `kit.alias` is discarded rather than just that one entry. Every literal alias declared alongside it stops
