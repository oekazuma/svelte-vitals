...
| `--config <config>`                     | Path to a config file to use instead of the one in the analyzed directory (relative paths resolve against the current shell directory, not the analyzed one) |
