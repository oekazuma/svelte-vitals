...
- **Source analysis** (the CLI, the dashboard's static baseline) walks the route's `.svelte` templates, including headings rendered by imported local components (followed transitively, depth-limited, the same traversal used for head resolution), so extracting a page's `<h1>` into a `$lib` component is credited. The arms of one `{#if}`/`{:else if}`/`{:else}` or `{#await}` block never render together, so within a file only the arm with the most `<h1>`s counts. Separate blocks still add up, because both conditions may hold, and a component placed in one arm counts as if it rendered unconditionally. It still counts headings in branches that may not render (e.g. inside `{#if false}`), and it cannot see components from `node_modules` or dynamically chosen components. A heading inside an unresolvable component can still produce a false "Missing `<h1>`", and `<h1>`s in separate conditional blocks, or in components placed in exclusive arms, can produce an `info`-level over-count.
