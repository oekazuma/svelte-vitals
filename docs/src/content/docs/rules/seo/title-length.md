...
**Source analysis** (the CLI, the dashboard's static baseline) measures the literal text you wrote in your source (or a static `title` prop on `svelte-meta-tags`/`svelte-seo`, or a static `title` in svead's `seo_config`). A `<title>` inside an `{#if}`, `{#each}` or `{#await}` block, or in a component placed in one, may not render, so it is not measured, and a `titleTemplate` is not measured, since the wrapped title only takes its final form at render time. **Rendered analysis** (the Vite plugin's build pass, a route you visit in the dashboard) measures the shipped text; the build pass covers prerendered routes only. The dashboard's live layer runs on the hook's own options, so a `min`/`max` set in the config file applies to the static baseline only.
