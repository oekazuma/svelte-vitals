...
- **Source analysis** (the CLI, the dashboard's static baseline) walks the route's `.svelte` templates, so it counts headings in branches that may not render (e.g. inside `{#if false}`) and cannot see headings rendered by imported child components. Each arm of one `{#if}`/`{:else if}`/`{:else}` or `{#await}` block is read on its own, as are a `<svelte:boundary>`'s content and its `failed`/`pending` snippets: a heading is compared with the nearest earlier heading that can render alongside it, never with one from a sibling arm. The page is read inside the arm of its layout's `<slot />`/`{@render children()}`, and content passed to a component inside the arm where that component renders it. A snippet the file renders is read at each `{@render}` of it, in that position of the document order. Separate blocks are treated as all rendering.
