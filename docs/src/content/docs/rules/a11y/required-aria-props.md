...
- A fallback list of roles (`role="switch checkbox"`) — only the first token would actually apply at runtime, so checking either reading risks a false positive; the rule skips fallback lists entirely.
