...
- Any attribute on an element `a11y/deprecated-element` reports (`<font color>`, `<marquee behavior>`) — one finding per element. That holds even when `a11y/deprecated-element` is off or suppressed inline: this rule skips obsolete elements by name.
