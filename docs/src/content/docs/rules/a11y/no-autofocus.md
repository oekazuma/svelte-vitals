...
description: autofocus moves focus on page load without the user asking, so screen reader users lose the page context they were building and keyboard users are dropped mid-page.
---
...
- An expression-valued `autofocus={expr}`, since the expression could be `false` and the rendered attribute is unknowable statically.
...
- An `autofocus` supplied through a spread attribute, which is out of static reach.
...
The compiler warns on similar markup as `a11y_autofocus`, with the same `<dialog>` carve-out. That overlap is deliberate: the compiler streams into the build log and does not score, gate, or suppress, while this rule feeds the health score, respects `svelte-vitals-disable-next-line`, and fails CI through `--fail-on`.

Four deliberate divergences remain:

- An expression-valued `autofocus={expr}` makes the compiler warn while this rule stays silent, since the expression could be `false`.
- A deep descendant of `<dialog>` (e.g. `<dialog><div><input autofocus /></div></dialog>`) makes the compiler warn, since it only checks the nearest element ancestor, while this rule walks the whole chain and correctly passes it.
