...
- **Source analysis** (the CLI, the dashboard's static baseline) also decides whether a `<header>`/`<footer>` is a landmark by where it sits in its own file: it counts as `banner`/`contentinfo` only at the template top level of a **chain file** (a layout or the page), never inside a component, whose header may sit under sectioning content the component cannot see. A chain file the layout above renders inside `<main>` or `<aside>` is scoped by it, so its top-level `<header>`/`<footer>` is not a landmark there either. This is an approximation: a layout that wraps the page in `<article>`, `<section>` or `<nav>` instead is not seen, so a page's top-level `<header>` still counts there, although the rendered DOM would not treat it as a landmark.
