...
- **Source analysis** (the CLI, the dashboard's static baseline) composes the route's layout chain with its resolved local components to detect nesting. It cannot see nesting introduced by an unresolvable component (`node_modules`, a component picked from data or a lookup table at runtime).
- **Source analysis** (the CLI, the dashboard's static baseline) also decides whether a `<header>`/`<footer>` is a landmark by where it sits in its own file: it counts as `banner`/`contentinfo` only at the template top level of a **chain file** (a layout or the page), never inside a component, whose header may sit under sectioning content the component cannot see. A chain file the layout above renders inside `<main>` or `<aside>` is scoped by it, so its top-level `<header>`/`<footer>` is not a landmark there either. This is an approximation: a layout that wraps the page in `<article>`, `<section>` or `<nav>` instead is not seen, so a page's top-level `<header>` still counts there, although the rendered DOM would not treat it as a landmark. A `<header>`/`<footer>` inside sectioning content, in its own file or through the layout's `<main>`/`<aside>`, does not contain a landmark either: an `<aside>` inside it is reported as nested in the landmark around it, if any.
