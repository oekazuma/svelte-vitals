...
A blank literal `title=""` carries no expansion and is also flagged. An expression-valued `title={expr}` passes — its rendered value is unknowable, so the attribute's presence is assumed to carry one.

Not flagged:

- An `<abbr>` with a spread attribute — its rendered attributes are unknowable.
- An `<abbr>` inside `<svg>` — it is not an SVG element and never renders there.
...
An expansion provided in the surrounding text — `The World Wide Web Consortium (<abbr>W3C</abbr>)` — is conforming markup this rule cannot see, and is the known false-positive class. Silence those with the inline directive below rather than adding a redundant `title`.
