...
- Namespace is not tracked here: an `<a href>` inside `<svg>` is judged as the HTML `<a>`, which means an SVG link with no `<title>` or `aria-label` is reported — the same thing the SVG accessibility guidance asks for, so the verdict stands even though the reason differs.
