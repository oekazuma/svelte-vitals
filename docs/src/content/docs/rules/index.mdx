...
<CardGroup cols={2}>
  <Card title="SEO" icon="search" href="/rules/seo">
    Resolved &lt;head&gt; metadata, structured data, and crawlability — what search engines actually see. (31 rules)
  </Card>
  <Card title="Performance" icon="zap" href="/rules/performance">
    Images, render-blocking assets, imports, and load waterfalls — what makes a route slow. (14 rules)
  </Card>
  <Card title="Correctness" icon="circle-check" href="/rules/correctness">
    Svelte 5 runes and lifecycle misuse — code that compiles but behaves wrong. (14 rules)
  </Card>
  <Card title="Security" icon="shield" href="/rules/security">
    Unescaped HTML, unsafe URLs, and server state that leaks across requests. (5 rules)
  </Card>
  <Card title="Architecture" icon="layers" href="/rules/architecture">
    Component size and prop surface — signals that a component is doing too much. (2 rules)
  </Card>
</CardGroup>
...
| [`security/server-module-state`](/rules/security/server-module-state) | 🟡 warning  | A module-scope variable in a Kit server file is reassigned from a function — shared across all requests.                      |
