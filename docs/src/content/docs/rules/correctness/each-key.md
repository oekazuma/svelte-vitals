...
- The same literal given a name in the component's own script (`const tabs = [ … ]`, not exported), when nothing in the file can change it: every other mention is a member read (`tabs.length`) or an `{#each tabs …}`. A mutating call (`tabs.push`), an assignment through it, or passing or spreading `tabs` keeps it reported.
- The same literal imported from a module in the project (`export const TABS = [ … ]`, `as const` and `satisfies` included), reached through `$lib` or another `kit.alias`, a relative path, or a barrel's re-exports, and iterated as `TABS` or as `ns.TABS` of `import * as ns`. The module must not write it, and the component is held to the reads above. A list from a package, a JSON file, or an export that is not an array literal (`LIST.map(…)`, spreads) is still reported.
