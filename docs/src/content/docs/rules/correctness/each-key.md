...
- The same literal given a name in the component's own script (`const tabs = [ … ]`, not exported), when nothing in the file can change it: every other mention is a member read (`tabs.length`), an `{#each tabs …}` or a `for (const t of tabs)` loop. A mutating call (`tabs.push`), an assignment through it, or passing or spreading `tabs` keeps it reported.
