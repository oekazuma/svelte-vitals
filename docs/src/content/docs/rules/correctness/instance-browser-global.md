...
description: A component's instance script runs on the server during SSR, so window/document reads at its top level crash the render.
---
...
Flags reads of browser-only globals (the same list as [correctness/server-browser-global](/rules/correctness/server-browser-global)) at the **top level of a component's `<script>`**, since that code runs on the server on every SSR render of the component. The same guards apply: `browser` from `$app/environment` (or `$app/env`), `typeof` checks (early-return guards included), `onMount`/`$effect` bodies, your own bindings, and shadowed locals are never flagged.
