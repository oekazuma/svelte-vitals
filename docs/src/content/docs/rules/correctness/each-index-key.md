...
- **Constant lists** (an inline array literal, a `const` array literal in the component's own script that nothing in the file can change, or one imported from a module in the project that does not write it; see `correctness/each-key`), which never reorder, so the index is a stable identity. Writes to an imported list from other modules that import it are not checked.
