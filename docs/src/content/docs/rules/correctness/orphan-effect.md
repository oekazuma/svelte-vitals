...
A conditionally-guarded effect is still flagged, whether it sits behind a top-level `if` or behind a constructor-argument check (`constructor(persist) { if (persist) $effect(...) }`), even if the guard is never true at runtime, because the guard can't be evaluated statically. Use an inline suppression (`svelte-vitals-disable-next-line correctness/orphan-effect`) if the guard is intentional.
...
Either create a standalone reactive scope with `$effect.root`, which is fine when the effect should live for the whole app. Own the returned cleanup function if it shouldn't:
