...
Flags a value `import * as X from '<package>'` from a bare (node_modules) package when `X` is used as a whole somewhere in the file: indexed by a variable (`X[key]`), passed on (`fn(X)`), spread, or enumerated (`Object.keys(X)`). A namespace read only through static member access (`X.foo()`, `X['foo']`, `{X.foo}` or `<X.Component />` in markup) tree-shakes like named imports and is not flagged. A local of the same name that shadows `X` (a function parameter, an `{#each}` item or index, a snippet parameter, an `{@const}`, a `let:` directive) is not a use of the namespace. Type-only imports (`import type * as T`) and non-bare specifiers (relative, `$lib`, `$app`, `$env`, `#…`) are not flagged.
