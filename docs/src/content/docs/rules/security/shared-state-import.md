...
Not flagged: a universal `+page.ts`/`+layout.ts` file that itself exports `ssr = false`, or any universal file when SSR is off app-wide: a root `src/routes/+layout.ts` (or `+layout.server.ts`) that exports `ssr = false`, as long as no other `+page`/`+layout` file exports `ssr` with any other value. SvelteKit's state-management docs: "If you're not using SSR, then there's no risk of accidentally exposing one user's data to another." That file's `load` never runs on the server, so there's no shared server instance to leak through. The exemption covers universal files only, so a `+page.server.ts` (always server-executed, regardless of `ssr`) still gets flagged.
