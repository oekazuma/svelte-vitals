...
- the server code **mutates** the imported state, outside handlers (`security/handler-state-write` reports handler writes as critical), or
- the import is **read-only**, and on the server the state is still one shared instance that keeps its boot-time value.
...
Not flagged: a universal `+page.ts`/`+layout.ts` file that itself exports `ssr = false`, or one whose route never renders on the server. A route never renders on the server when the nearest `ssr` export, looking first at the page's own `+page` files and then up its layout chain (following `+page@`/`+layout@` resets), is `ssr = false`. An `ssr` exported as anything but a literal `false` counts as turning SSR back on. A layout qualifies only when every page under it does, and the root layout only when it exports `ssr = false` itself. SvelteKit's state-management docs: "If you're not using SSR, then there's no risk of accidentally exposing one user's data to another." That file's `load` never runs on the server, so there's no shared server instance to leak through. The exemption covers universal files only, so a `+page.server.ts` (always server-executed, regardless of `ssr`) still gets flagged.
...
If the module is genuinely client-only, restructure so server files don't import it. If the import is deliberate and safe, add `// svelte-vitals-disable-next-line security/shared-state-import` above it instead.
