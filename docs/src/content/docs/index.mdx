---
title: svelte-vitals
description: A static code-health checker for SvelteKit — SEO, Performance, Correctness, Security, Architecture, and Accessibility, from source.
---

Your SvelteKit app's health — SEO, Performance, Correctness, Security, Architecture, and Accessibility — checked before you deploy.
...
svelte-vitals reads your source: it walks the layout chain to resolve each route's effective `<head>`, and parses component bodies for runes, `{@html}`, images and ARIA. It scores your app across six categories — SEO, Performance, Correctness, Security, Architecture, and Accessibility — per route and site-wide, and gates CI when a score falls below your threshold. Add the [Vite plugin](/guides/plugin-mode) and the same rules run against the real HTML your build ships, where every dynamic value is already resolved.
