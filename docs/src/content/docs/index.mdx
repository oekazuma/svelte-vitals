...
svelte-vitals statically resolves each route's effective `<head>` and `<img>` usage from source, then scores your app across six categories — SEO, Performance, Correctness, Security, Architecture, and Accessibility — per route and site-wide. Fall below a threshold and it gates CI. All of it, before you ever deploy.
