...
**Spec coverage.** Design testing items map as: 1 → Task 2 test 1; 2 → Task 2 test 2; 3 → Task 2 test 3; 4 → Task 2 test 4; 5 → Task 2's rewritten ordering test; 6 → Task 1 test 1; 7 → Task 2 test 6; 8 → Task 2 test 5; 9 → Task 2 test 7; 10 → covered by the existing aggregation test, which Task 2 Step 5 requires to keep passing. The design's "must not say matched" constraint is in Global Constraints. It was not enforced when this was written: `toContain('reaches no unit')` alone is satisfied by a message carrying both the new wording and the retired one. The unit-only fixtures in `packages/core/test/reserved-name-placement.test.ts` now pair that assertion with `not.toContain('matched directories but never a unit')`, verified to fail when the retired phrase is reintroduced. Dead-code removal is Task 2 Step 4 and verified by Task 2 Step 7's grep.
