...
- **Environment — pnpm run is BROKEN in this sandbox (pnpm 11.15 purge bug). NEVER run any `pnpm` command and NEVER any install.** Use direct binaries instead: tests `cd packages/core && ../../node_modules/.bin/vitest run <pattern>`; core build `cd packages/core && ../../node_modules/.bin/tsup`; typecheck `cd packages/<pkg> && ../../node_modules/.bin/tsc --noEmit` (check the package's own typecheck script for flags first); prettier/eslint via `node_modules/.bin/...` from the repo root. If a step genuinely requires pnpm (e.g. a cross-package script), STOP and report it instead of running pnpm.
- `node_modules/.bin/prettier --write` on every touched file before each commit.
...
function collectStateWrites(
  ...
  walkScoped(root, (n: Node, scope: Set<string>) => {
    const shadowed = (name: string | undefined): boolean =>
      name === undefined || scope.has(name);
    if (n?.type === "AssignmentExpression") {
      if (
        n.left?.type === "Identifier" &&
        stateNames.has(n.left.name) &&
        !shadowed(n.left.name)
      ) {
        ...
        n.left?.type === "ObjectPattern" ||
        n.left?.type === "ArrayPattern"
      ) {
        const bound = new Set<string>();
        addBoundNames(n.left, bound);
        for (const name of bound)
          if (stateNames.has(name) && !shadowed(name)) record(name, "reassign");
      }
    } else if (n?.type === "UpdateExpression") {
      if (n.argument?.type === "Identifier") {
        if (stateNames.has(n.argument.name) && !shadowed(n.argument.name))
          record(n.argument.name, "reassign");
      } else {
...
```bash
node_modules/.bin/prettier --write packages/core/src/component-parse.ts
...
const constable = (src: string) =>
  parseComponentFacts(src, "A.svelte").constableStates;

const script = (body: string, template = "<p>x</p>") =>
  `<script>\n${body}\n</script>\n${template}`;
...
describe("rawableStates — records", () => {
  ...
  it("records when the reassignment happens in a template inline handler", () => {
    const src = script(
      `let list = $state([]);`,
      "<button onclick={() => (list = [...list, 1])}>x</button>",
    );
    expect(raw(src)).toEqual([{ name: "list", line: 2 }]);
  });
...
describe("rawableStates — exclusions", () => {
  ...
  it("excludes non-candidates: raw already, non-literal, primitive, non-top-level", () => {
    ...
    expect(
      raw(script(`let c = $state(0);\nfunction f() {\n  c = 1;\n}`)),
    ).toEqual([]);
    expect(
      raw(script(`function g() {\n  let d = $state({});\n  d = {};\n}`)),
    ).toEqual([]);
  });
 ...
 * non-computed member properties/keys, and `$state(...)` declarator arguments;
 * shadow-aware. Walks whatever subtree it is given — callers choose the roots.
 */
function collectAliasRefs(
  ...
  if (Array.isArray(node)) {
    for (const child of node)
      collectAliasRefs(child, names, acc, shadowed, ownRhs);
    return;
  }
  ...
  const scope =
    introduced.size > 0 ? new Set([...shadowed, ...introduced]) : shadowed;
  if (node.type === "AssignmentExpression") {
    const lhsIsCandidate =
      node.left?.type === "Identifier" &&
      names.has(node.left.name) &&
      !scope.has(node.left.name);
    ...
    collectAliasRefs(
      ...
      acc,
      scope,
      lhsIsCandidate ? node.left.name : null,
    );
    return;
  }
  if (
    ...
    node.init &&
    isPlainStateCall(node.init)
  )
    return; // the candidate's own declaration
  if (
...
function collectFragmentAliasRefs(
  ...
  if (Array.isArray(node)) {
    for (const child of node)
      collectFragmentAliasRefs(child, names, acc, shadowed);
    return;
  }
  if (!node || typeof node !== "object" || typeof node.type !== "string")
    return;
  ...
  if (Array.isArray(node.attributes))
    collectFragmentAliasRefs(node.attributes, names, acc, scope);
  for (const key of Object.keys(node)) {
...
function collectEachContextTaint(
  ...
  if (Array.isArray(node)) {
    for (const child of node)
      collectEachContextTaint(child, names, acc, shadowed);
    return;
  }
  if (!node || typeof node !== "object" || typeof node.type !== "string")
    return;
  ...
  if (node.type === "EachBlock") {
    ...
    if (
      ...
      if (ctxNames.size > 0) {
        ...
        collectStateWrites(node, ctxNames, union, kinds);
        collectTemplateEscapes(node, ctxNames, union, kinds);
...
for (const stmt of program.body ?? []) {
  ...
  for (const d of stmt.declarations ?? []) {
    if (d?.id?.type !== "Identifier" || !d.init || !isPlainStateCall(d.init))
      continue;
...
```bash
node_modules/.bin/prettier --write packages/core/src/component.ts packages/core/src/component-collect.ts packages/core/src/component-parse.ts packages/core/test/state-raw-parse.test.ts
...
function comp(
  file: string,
  rawableStates: ComponentFacts["rawableStates"],
): ComponentFacts {
...
describe("performance/state-raw", () => {
  it("suggests $state.raw per candidate at info severity", async () => {
    const results = await performanceStateRaw.check(
      ctx([comp("src/lib/Feed.svelte", [{ name: "posts", line: 4 }])]),
    );
  ...
  it("emits nothing without candidates", async () => {
    expect(
      await performanceStateRaw.check(ctx([comp("src/lib/Ok.svelte", [])])),
    ).toEqual([]);
  });
...
export const performanceStateRaw = componentRule({
  ...
  fix: {
    description:
      "Replace $state(...) with $state.raw(...); keep the same initializer.",
  },
...
```bash
node_modules/.bin/prettier --write packages/core/src/rules/perf/state-raw.ts packages/core/src/rules/index.ts packages/core/src/index.ts packages/core/test/state-raw-rule.test.ts
...
Then: `cd packages/cli && ../../node_modules/.bin/vitest run` (docs-links now green) and `../../node_modules/.bin/vitest run` in core and vite; `node_modules/.bin/prettier --check .` and `node_modules/.bin/eslint .` from the root (expect only the 2 pre-existing meta-object warnings); `cd packages/<pkg> && ../../node_modules/.bin/tsc --noEmit` for core/cli/vite/mcp; `cd docs && node_modules/.bin/astro build`. Check `git status --short packages/action/dist` and commit the rebuilt dist.
