...
4. **Never escaped**: not passed as a call argument, not `bind:`-bound (v1 treats even a whole-binding `bind:` as disqualifying — conservative; a bound member is mutation anyway), not passed as a component prop, not passed to a directive expression (`use:`/`transition:`/`animate:` — a new-rule-only collector, keeping the shared template-escape collector unchanged for unmutated-state), and — closing the aliasing hole — no bare script-side reference OUTSIDE the candidate's own reassignments: a reference in a declarator initializer (`const inner = obj.items`), in the RHS of an assignment to a DIFFERENT target, in a `return`, or anywhere else in the instance program that is not the LHS or RHS of an assignment to the candidate itself, classifies as `escape`. The immutable-update idiom `list = [...list, x]` stays a qualifying reassign — the own-RHS exemption applies only when the NEAREST enclosing assignment's LHS is the candidate itself, so `obj = (cache = obj)` and `obj = { cb: () => (cache = obj) }` still escape (their nearest assignment targets a different binding). Template READS (`{obj.title}`, `{#each list as item}`) do not escape, but the aliasing scan DOES cover function bodies in the fragment (`onclick={() => (cache = obj)}` escapes) — only true read positions (expression tags, attribute/block expressions outside function bodies) are exempt.
5. **Each-context taint**: for `{#each candidate as item}` blocks over the candidate — or a member path of the candidate (`{#each obj.items as item}` — resolved via its root object) — the context binding (and index) is tracked within the block (shadow-aware): any mutation, `bind:`, method call, call-argument escape, or component-prop pass of the context name disqualifies the candidate — an editable/optimistic list (`bind:value={item.text}`, `<Row {item} />`) must NOT be told to go raw, because item-level edits stop being reactive under `$state.raw`.
...
Two additional collectors are new-rule-only (they do NOT feed the union): a bare-reference scan implementing condition 4's aliasing clause over the instance program AND fragment function bodies (shadow-aware; skips references inside the candidate's own `$state(...)` declarator and own-reassignment LHS/RHS under nearest-enclosing-assignment semantics), and the each-context taint of condition 5 over the fragment.
...
- `use:action={state}` arguably should escape for `unmutated-state` too (a directive can mutate or rely on the reference's identity same as a call argument) — recorded here as a candidate follow-up, deliberately NOT changed on this branch: the shared `collectTemplateEscapes` collector stays untouched so `correctness/unmutated-state` behavior does not shift mid-branch.
