...
1. **Identifier binding** (destructuring-pattern declarators are skipped — conservative), with an initializer whose expression **references at least one prop name**. Prop names come from a small variant of the existing `collectNonBindableProps` collector that ALSO includes `$bindable(...)`-defaulted names (a `$bindable` prop is just as reactive, so deriving from it is just as stale); like the original, it includes the `...rest` binding and the whole-object name from `let props = $props()` (deriving from `rest.foo` is frozen too), and yields nothing for nested patterns or multiple `$props()` calls (rule inert — conservative). Reference collection is shadow-aware (nested-function params) and ignores non-computed member property names and object keys. A bare alias (`const t = type;`) qualifies. **Eager positions only**: prop references occurring solely inside nested function/arrow bodies or object getters do NOT count — the compiler defers those reads to call time, so `const getLabel = () => type;` and `const o = { get label() { return type; } }` stay reactive and must not be flagged (verified against compiled runes output).
...
3. **Never reassigned**: the binding is not written anywhere in the component (script assignments, update/compound operators, or template two-way bindings). Reuse the existing `collectStateWrites` machinery over the program and fragment (it is parameterized over an arbitrary name set) AND `collectTemplateEscapes` over the fragment — the `bind:` two-way-binding disqualifier lives there, not in `collectStateWrites`. Decision: accept its write-OR-escape semantics as-is — a binding that escapes (passed to a call, method-called) is also disqualified. That over-disqualifies (`{fmt(color)}` in the template hides `color` from the rule — a false negative), which is the safe direction and keeps the machinery shared.
4. **Referenced in the template**: the binding's name is read at least once in the component's fragment (expression tags, attribute/directive expressions, block expressions), shadow-aware for template scopes. `scopeIntroducedNames` today models only `{#each}` CONTEXTS (not the index) among template constructs — this work extends it additively with `EachBlock.index`, `SnippetBlock.parameters`, and `AwaitBlock.value`/`AwaitBlock.error` so snippet/await bindings that shadow a candidate name do not count as references (safe for its existing consumers: more shadowing only makes them more conservative). Bindings used only inside handlers/functions are NOT flagged (lower confidence; often intentional captures). The template walk applies the same eager principle: references inside inline-handler function bodies (`onclick={() => alert(color)}`) do not count as template references, while `{#snippet}` bodies DO (they are render content, not deferred reads).
...
`$state(prop)` / any rune-wrapped initializer (structural, via call-free); initializers containing any call/`new`/`await`; reassigned bindings; bindings not referenced in the template; destructuring declarators; components with nested `$props()` patterns or multiple `$props()` calls (no prop names collected → rule inert; note `...rest` IS collected, so `const x = rest.foo` is flagged); `<script module>` declarations; props read directly in the template (fine — that's reactive).
...
- Interplay: `correctness/unmutated-state` currently recommends "use const" for never-written `$state` — advice that, applied to `let color = $state(type === …)`, produces exactly this rule's "don't" pattern. This branch also tweaks that recommendation string to "…use const — or $derived if it is computed from props or state", and the new rule's doc page notes the interplay.

## Testing

- **Parse unit**: the official do/don't pair (don't-form flagged at its line; `$derived` form not); bare alias `const t = type;` flagged; `$state(type)` not; `buildConfig(type)` / `new Thing(type)` / `await x` not (call-free); reassigned `let` not; binding used only in a handler not; binding used in an `{#each}` body where the context shadows the name → shadowing respected both ways; renamed prop (`{ type: kind }`) flagged via `kind`; nested patterns or multiple `$props()` calls → nothing, while `const x = rest.foo` from a `...rest` binding IS flagged; `<script module>` declaration → nothing; template usage via attribute expression and via block expression both count; closure/getter references NOT flagged (`const f = () => type;`, `const o = { get l() { return type; } }`) while an eager object literal (`const o = { c: type };`) IS; `$bindable`-defaulted prop derivation flagged; snippet-param shadowing does not count as a template reference.
