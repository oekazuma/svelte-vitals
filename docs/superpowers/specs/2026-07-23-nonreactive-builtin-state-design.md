...
2. **Type-specific mutation observed** (shadow-aware, over the instance program AND the template fragment — inline handlers mutate in templates):
   - Mutating METHOD call on the binding (`b.<m>(...)`, non-computed callee), where `<m>` is in the type's mutation set:
     ...
     - `Date`: `setTime`, `setFullYear`, `setMonth`, `setDate`, `setHours`, `setMinutes`, `setSeconds`, `setMilliseconds`, `setYear` (deprecated but implemented everywhere; SvelteDate intercepts it too), and the `setUTC*` variants
     - `URLSearchParams`: `append`, `set`, `delete`, `sort`
     - `URL`: (no mutating methods — property writes only)
   - OR — for `URL` bindings ONLY (URL is the one type that mutates via properties; an expando-property write on a Map/Set/Date/URLSearchParams is not something `Svelte<Type>` would track either, so flagging it would pair a correct diagnosis with a wrong remedy) — a member assignment / member `delete` / member `UpdateExpression` rooted at the binding (`url.href = …`, `url.searchParams.append` resolves via root — note `u.searchParams.append(...)` on a `URL` binding is a mutating call whose `rootObjectName` is the binding: treat any DEEP method call `b.<path>.<m>(...)` as a mutation only when the FINAL method is in the URLSearchParams mutation set or the access path includes `searchParams`; simpler v1 rule: for `URL` bindings, any member assignment or any method call at depth ≥ 2 rooted at the binding counts as mutation — precise enough because `URL` has no legitimate deep read-methods that common code calls besides `searchParams` accessors (`get`/`has` — those are reads!). DECISION: the deep-call rule applies to `URL` bindings ONLY — for Map/Set/Date/URLSearchParams bindings, only DIRECT calls `b.<m>(...)` count (so `m.get('k').sort()` never flags `m`). For URL bindings, a deep call counts only when the final method name is in the URLSearchParams mutation set; deep read methods (`get`, `has`, `getAll`, `toString`) never count.)

3. **Never reassigned after initialization**: no whole-binding reassignment (detection mirrors `collectStateWrites`'s reassign sites, implemented in the dedicated scanner because the bare-self-assignment exception below requires inspecting the RHS, which the kinds map does not carry) — the documented Svelte 5 workaround `m.set(k, v); m = new Map(m)` WORKS (fresh reference triggers the update) and must not be flagged; it becomes a documented false negative instead. EXCEPTION: a bare self-assignment `b = b` does not exempt — it is a no-op under `$state`'s referential equality in Svelte 5 (verified empirically), so the migrated-from-Svelte-4 `m.set(...); m = m` pattern stays flagged. Implementation: a reassign whose RHS is exactly the bare Identifier `b` is ignored when computing the exemption.
4. **The mutation happens inside a function body or a template inline handler** — a mutation at instance-script top level (`let d = $state(new Date()); d.setHours(0, 0, 0, 0);`) runs once before the first render, so the UI is never stale; the scanner threads an in-function flag (function/arrow/class boundaries, like the kit-module walker) and top-level mutations do not count.

Line recorded: the declarator's line. Reassign-only, read-only, escape-only, top-level-init-mutation-only, and mutate-then-fresh-reassign usage is NOT flagged (escapes and the fresh-reassign workaround are documented false-negative directions: code receiving the instance may mutate it out of sight, and a fresh reassignment after mutation genuinely works).
...
Reassign-only usage; mutate-then-fresh-reassign (`m = new Map(m)` — works, documented FN; bare `b = b` does NOT exempt); top-level init mutations (run before first render); read-only usage; escapes (call arguments, component props, directives — documented FN); `$state.raw(new X(...))`; `$derived(new X(...))` and non-`$state` constructions; nested-function-local `$state`; runes-module (`.svelte.ts`) and class-field `$state` (v1 scope, consistent with `state-raw`); locally shadowed re-uses of the binding name.
...
- **Parse unit**: `$state(new Map())` + `m.set(...)` in a handler → recorded with type `Map`; same for `Set.add`, `Date.setHours`, `URLSearchParams.append`, and `URL` via `u.href = ...` and `u.searchParams.set(...)`; `new Map(entries)` with args → recorded when mutated; template inline-handler mutation counts; reassign-only `$state(new Date())` → NOT recorded; read-only (`m.get`/`m.has`/`u.searchParams.get`) → not; escape-only (`register(m)`) → not; `$state.raw(new Map())` mutated → not; plain `const m = new Map()` (no `$state`) → not; shadowed local (`function f(m) { m.set(...) }`) → does not count as mutation of the outer binding; nested-function-local `$state` → not a candidate; mutate-then-fresh-reassign (`m.set(...); m = new Map(m)` in a handler) → NOT recorded; mutate + bare self-assign (`m.set(...); m = m;`) → recorded (self-assign is a no-op); top-level-only mutation (`d.setHours(0, 0, 0, 0);` at script top level) → NOT recorded, while the same call inside a handler IS.
