...
- **The dot-entry invariant is structural, not liftable.** Excluding dot directories must happen
  *during* traversal — a post-filter in core would mean walking `.git/` and `node_modules/`
  first. So core can own the policy only as an exported predicate that adapters apply; the
  enforcement point stays in the adapter either way. The win is small against the speculative
  need.
...
apply it, and move whatever invariants a post-collection check *can* verify into core so a
