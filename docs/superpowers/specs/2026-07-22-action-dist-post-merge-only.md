...
## Superseded (2026-07-23): the whole self-heal mechanism is gone — the action moved to its own repository

The same day the PR-based fix above landed, the private-package tagging step hit a second real incident: an orphaned commit from an earlier abandoned release-mechanism iteration collided on version number with a real release, and `action-v0.3.8`'s tag/release briefly pointed at stale dist content. Investigating _why_ this class of bug kept recurring (constant dist-rebuild churn, then a PAT/branch-protection incident, then a tag-collision incident — three separate incidents from the same root architecture) led to re-examining the architecture itself rather than patching it a third time.

Checking how well-known monorepo-hosted tools solve this (`changesets/action`, `pnpm/action-setup`, `renovatebot/github-action`) found that all three keep their GitHub Action in a **dedicated, separate repository** — not a subdirectory of the main monorepo. Their action repos depend on the main project's _published npm packages_ as regular semver-ranged dependencies (not workspace links), so there is no cross-repo dist-sync mechanism to build or maintain at all: the action repo's dist only changes when _it_ deliberately bumps its own dependencies, on its own schedule.

Separately, and just as importantly: a live `renovate` CLI dry run against the real `oekazuma/svelte-vitals` repo proved that Renovate's `github-actions` manager could **never** discover new `@svelte-vitals/action` releases in the monorepo, regardless of the tag-comment format — `versionLikeRe` (Renovate's own source) rejects any prefixed value like `action-v0.3.8`, routing it to a digest-only datasource that can't discover new tags at all. The `action-v<version>` comment fix from `2026-07-22-rule-id-eslint-style`-adjacent work (see PR #277) only fixed a regex crash, not the actual "Renovate never proposes an update" problem this repo originally set out to solve.

Both problems — dist-sync churn/incidents, and Renovate discoverability — share the same root cause: a monorepo's shared git-tag namespace. Moving the action to [oekazuma/svelte-vitals-action](https://github.com/oekazuma/svelte-vitals-action) (a single-package repo, plain `vX.Y.Z` tags, `svelte-vitals`/`@svelte-vitals/core` as regular npm dependencies) fixes both at once and was verified empirically (a local `renovate` CLI dry run against the real new repo, zero custom config, correctly resolved to the `github-tags` datasource).

Consequently, everything this document describes is now dead: `ci.yml`'s dist-aware caching comments, `release.yml`'s "Open a PR to refresh action dist if stale" step, and the "Tag and release private packages" step (including its ancestry-check hardening) were all removed from `oekazuma/svelte-vitals`'s workflows. The new repo has its own, much simpler release.yml (single package, no cross-package tag-naming translation needed) — see that repo's own history for its design.
