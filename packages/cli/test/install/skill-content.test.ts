...
describe('buildCursorRules', () => {
  ...
  const body = (text: string) => text.slice(text.indexOf('<!-- Generated by'));

  it('is the same rule body as the skill, under Cursor rules frontmatter', () => {
    expect(mdc).toMatch(/^---\ndescription: .+\nglobs: \[.+\]\nalwaysApply: false\n---\n/);
    expect(body(mdc)).toContain('svelte-vitals 1.2.3');
    expect(body(mdc)).toBe(body(md));
  });
});
