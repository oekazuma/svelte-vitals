...
describe('upgradeActionPin', () => {
  it('replaces a pin with a same-line version comment', () => {
    const content = [
      ...
      '  svelte-vitals:',
      '    steps:',
      `      - uses: oekazuma/svelte-vitals/packages/action@${OLD_SHA} # action-v1.0.0`,
      '        with:',
      '          diff: origin/main'
    ].join('\n');
    ...
    expect(outcome.content).toContain(
      `      - uses: oekazuma/svelte-vitals/packages/action@${NEW_SHA} # action-v2.0.0`
    );
  ...
  it('upgrades a pin still using the pre-fix npm-scoped comment format (# @svelte-vitals/action@X.Y.Z)', () => {
  ...
  it('reports up-to-date when the pin already matches', () => {
    const content = `      - uses: oekazuma/svelte-vitals/packages/action@${NEW_SHA} # action-v2.0.0`;
    const outcome = upgradeActionPin(content, NEW_SHA, '2.0.0');

    expect(outcome).toEqual({ status: 'up-to-date' });
  });

  it('normalizes a pre-fix comment even when the sha is already current (Renovate needs the new comment shape)', () => {
  ...
  it('upgrades a CRLF workflow with no trailing comment, keeping the line CRLF-terminated', () => {
    ...
    expect(outcome.content).toBe(
      [`      - uses: oekazuma/svelte-vitals/packages/action@${NEW_SHA} # action-v2.0.0`, 'next line'].join('\r\n')
    );
  });

  it('reports up-to-date (not no-reference) for a CRLF workflow already pinned to the current sha', () => {
    const content = [`      - uses: oekazuma/svelte-vitals/packages/action@${NEW_SHA} # action-v2.0.0`, ''].join(
      '\r\n'
    );
  ...
  it('rewrites an anchor-defined uses: line, preserving the anchor name', () => {
    const content = [
      ...
      '  a:',
      '    steps:',
      `      - uses: &vitals_action oekazuma/svelte-vitals/packages/action@${OLD_SHA} # action-v1.0.0`,
      '  b:',
      '    steps:',
      '      - uses: *vitals_action'
    ].join('\n');
    ...
    expect(outcome.from).toBe('1.0.0');
    expect(outcome.content).toContain(
      `      - uses: &vitals_action oekazuma/svelte-vitals/packages/action@${NEW_SHA} # action-v2.0.0`
    );
  ...
  it('reports up-to-date for an anchor-defined line already pinned to the current sha', () => {
    const content = `      - uses: &vitals_action oekazuma/svelte-vitals/packages/action@${NEW_SHA} # action-v2.0.0`;
    const outcome = upgradeActionPin(content, NEW_SHA, '2.0.0');
