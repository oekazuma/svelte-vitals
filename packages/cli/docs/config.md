...
plugin resolves its own config from the project root — share with it by importing the shared file
in `vite.config.ts` and spreading it into the plugin's options.
