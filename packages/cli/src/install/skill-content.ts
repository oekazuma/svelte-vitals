...
4. For anything else — reporters, the config file, scoping to a change, CI, monorepos — run \`npx svelte-vitals docs list\` and then \`docs show <name>\`. Those guides ship inside the CLI, so they match the version installed here; prefer them over searching the web.
