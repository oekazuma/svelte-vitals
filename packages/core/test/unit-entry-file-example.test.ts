...
import { architectureUnitEntryFile } from '../src/internal.js';
...
describe('the documented example configuration', () => {
  it('reports nothing on a compliant tree while examining every declaration it makes', async () => {
    ...
    // `location` is what identifies which entry file each pass examined. One assertion per
    // declaration key, so a key that silently matches nothing fails here.
    ...
    // own name appears only in the message. The api domain level, reserved folders, camelCase
    // groupings and matcher segments must stay untouched.
    ...
    // A project-scoped inert finding carries neither `route` nor `location`; the compliant tree's
    // passes carry `location` alone, so `route === undefined` alone would wrongly catch them too.
