...
export const securityServerModuleState = kitModuleRule({
  ...
  // A load that never runs on the server keeps its module state per browser tab, not per server process.
  applies: (m, ctx) => m.moduleStateReassignments.length > 0 && !universalNeverSsr(m, ctx),
  bad: (m) =>
