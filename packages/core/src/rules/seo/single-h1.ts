 ...
 * channel (collected by both providers). Zero <h1> (no primary heading) is a `warning`:
 * defensible, a page needs a primary heading. Two or more is only `info`: a single <h1>
 * is the conventional signal, but no official source documents a ranking penalty for
 * several (2026-08-09 v1 rule-validity review, P2 #11) — so it's flagged as a style
 * nit, not a defect. Exactly one passes. A route whose headings were not collected
 * (channel unset) emits nothing. A global `rules: { 'seo/single-h1': <severity> }`
 * override flattens both arms to one severity (design, `applyRuleSeverities`).
 */
