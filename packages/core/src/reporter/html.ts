...
// Human label for a category key; reused by the hero bars and the filter chips
// so a new category surfaces in both places without a second edit.
const categoryLabel = (cat: string): string => (cat === 'seo' ? 'SEO' : cat.charAt(0).toUpperCase() + cat.slice(1));
...
function renderFinding(issue: Issue): string {
  // Clamp to the known set: `buildHtmlDocument` is public and severity flows into
  // class/data attributes, so a malformed value must never reach the markup.
  const sev = (SEVERITIES as readonly string[]).includes(issue.severity) ? issue.severity : 'info';
  const dyn =
...
const ROUTE_BADGES = ['measured', 'static'] as const;
type RouteBadge = (typeof ROUTE_BADGES)[number];

function renderRoutes(report: JsonReport, routeBadges?: Record<string, RouteBadge>): string {
  ...
  const rows = report.routes
    .map((r) => {
      ...
      // Clamp to the known set, same reasoning as `renderFinding`'s severity clamp:
      // an arbitrary caller-supplied value must never reach the class/text markup.
      const rawBadge = routeBadges?.[r.route];
      const badge = rawBadge && (ROUTE_BADGES as readonly string[]).includes(rawBadge) ? rawBadge : undefined;
      ...
      return (
        ...
        `<summary><span class="route-name"><span class="path">${escapeHtml(r.route)}</span>${badgeHtml}</span>` +
...
function renderFilters(report: JsonReport): string {
  // `filter` is the value the script matches against data-severity/data-category;
  // `label` is the visible text. Decoupling them means the chip works even when
  // the label's casing/spacing differs from the underlying key.
  const chip = (filter: string, label: string, pressed = false) =>
    `<button class="chip" type="button" aria-pressed="${pressed}" data-filter="${escapeHtml(filter)}">${escapeHtml(label)}</button>`;
  const catChips = Object.keys(report.categories)
    .map((cat) => chip(cat, categoryLabel(cat)))
    .join('');
  return (
...
export function buildHtmlDocument(
  report: JsonReport,
  meta: { version: string },
  opts?: { routeBadges?: Record<string, 'measured' | 'static'> }
): string {
  return (
