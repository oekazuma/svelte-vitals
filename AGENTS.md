...
| Corpus         | `pnpm corpus update` | manual, needs `pnpm build` first — measures every rule on the pinned apps in `scripts/corpus/targets.json`, joins `scripts/corpus/verdicts.json`, and rewrites `measurement.json` plus the table in `scripts/corpus/README.md` (then `pnpm format`). `run`/`diff` are what the `corpus.yml` PR job uses. See `docs/superpowers/specs/2026-09-23-corpus-precision-design.md`         |
...
CI (`.github/workflows/ci.yml`) runs five jobs: `lint`, `check` (build + typecheck + check:publish), `test`, `floor-smoke`, `docs`. A separate `.github/workflows/ecosystem.yml` runs the ecosystem smoke weekly. `.github/workflows/corpus.yml` measures each PR that touches `packages/**` against its base on the pinned corpus and posts the finding diff and precision as a sticky comment; its `gate` job fails when a `tp`-verdict finding disappears, an `fp`-verdict finding returns, or `measurement.json` is stale (change the verdict in `scripts/corpus/verdicts.json` in the same PR when that is intended). Run the relevant verify commands yourself and confirm they pass **before** claiming a task is complete.
