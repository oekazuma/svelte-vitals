...
| Corpus         | `pnpm corpus update` | manual, needs `pnpm build` first — measures every rule on the pinned apps in `scripts/corpus/targets.json`, joins `scripts/corpus/verdicts.json`, and rewrites `measurement.json` plus the docs "Rule reliability" tables (then `pnpm format` and re-stamp the page). `run`/`diff` are what the non-blocking `corpus.yml` PR job uses. See `docs/superpowers/specs/2026-09-23-corpus-precision-design.md` |
