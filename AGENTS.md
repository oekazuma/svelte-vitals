...
| Lint           | `pnpm lint`          | `oxlint .` + `oxfmt --check .` + `pnpm lint:docs` (textlint `@textlint-ja/preset-ai-writing` over READMEs, `packages/cli/docs/`, and the docs site — config in `.textlintrc.json`)                      |
